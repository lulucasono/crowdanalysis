...
def SimilarityOfLayer(seq_list, i, j):
	...
	s = 0;
	for seq in seq_list[input_seq.iloc[j,0]]:
...
similarity = [0 for i in range(max_id_of_people+1)]
for i in range(len(input_seq[0])):
	pid = input_seq.iloc[i,0]
	if (pid!=2) & (pid!=24) & (pid!=1) & (pid!=int(person_id)):
		print("=============================================")
		print("perseon id:",	pid)
		max_len_seq_table[pid] = SequenceMatching(input_seq,person_idx,i,max_length,tth)
		similarity[pid] = SimilarityOfLayer(max_len_seq_table, person_idx, i)
		print(person_id,"-",input_seq.iloc[i,0],":",max_len_seq_table[input_seq.iloc[i,0]])
		print("Similarity("+str(person_id)+","+str(pid)+"):",similarity[pid])
